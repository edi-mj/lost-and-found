...
<div class="pt-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    ...
    <h3 class="text-2xl font-bold mb-4">Barang Ditemukan</h3>

    <div id="found-list" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6"></div>
...
async function loadFound() {
    ...
    try {
        const res = await fetch("{{ config('services.backend_api.reports') }}/reports/type/found", {
        ...
        if(!data || data.length === 0){
            container.innerHTML = `<p class="text-gray-500 col-span-full text-center">Belum ada laporan barang ditemukan.</p>`;
            return;
        }
        ...
        data.forEach(item => {
            ...
            div.innerHTML = `
                <img src="{{ config('services.backend_api.reports') }}${item.photo_url || ''}" class="h-48 w-full object-cover mb-2 rounded-md">
                <h4 class="font-bold text-lg">${item.name || 'Nama Barang Tidak Diketahui'}</h4>
                <p class="text-sm text-gray-500 mt-1">${item.description}</p>
                <p class="mt-1 text-xs text-gray-400">${item.location} | ${date}</p>
                <span class="inline-block mt-2 px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Ditemukan</span>
                <button data-report-id="${item.id}" class="detail-btn mt-3 px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 text-sm font-medium">
                    Detail
                </button>
            `;
        ...
        document.querySelectorAll('.detail-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                try {
                    const res = await fetch(`{{ config('services.backend_api.reports') }}/reports/${id}`, {
                        headers: { 'Authorization': 'Bearer ' + token }
                    });
                    ...
                    modalContent.innerHTML = `
                        ...
                        <img src="{{ config('services.backend_api.reports') }}${report.photo_url || ''}" class="w-full h-64 object-cover rounded-lg mb-4">
        ...
        console.error(err);
        const container = document.getElementById('found-list');
        container.innerHTML = `<p class="text-red-500 text-center col-span-full">Gagal memuat data barang ditemukan.</p>`;
    }
}

document.addEventListener('DOMContentLoaded', loadFound);
</script>
